...
**Priorité : haute.** Objectif final confirmé : que le Heltec V4 devienne
l'observateur MQTT **principal et fiable**, fonctionnant en autonomie
24h/24 (alimentation solaire), afin de ne plus dépendre du téléphone
(app KiekR) que comme solution de repli occasionnelle. Le bug de
reconnexion (perte de données probable à chaque coupure, ~4-5s sur 5)
empêche actuellement d'atteindre cette fiabilité — à résoudre avant de
pouvoir compter sur le Heltec comme source principale.
...
- ✅ Convention de nommage CLI choisie : `mqtt1.server` / `mqtt1.port` /
  `mqtt1.topic` (au lieu de `mqtt.*`, déjà pris par le broker principal
  `mqtt.no-ip.org`) — à utiliser quand ce second bridge sera implémenté.
  **Pas encore fonctionnel** : ne pas ajouter ces commandes à
  l'autocomplétion de la page web tant que le vrai second bridge n'existe
  pas (ça afficherait "Unknown command" et donnerait une fausse impression
  de bug).
...
## Solution pragmatique appliquée (en attendant la vraie cause)
Plutôt que de continuer à chercher la cause profonde (qui nécessiterait des
outils plus lourds — JTAG, logs lwIP), un correctif pragmatique a été
appliqué : le délai de retry dans `MqttBridge::ensureConnected()` a été
réduit de **5000ms à 300ms**.

**Résultat observé** : le cycle "vie de la connexion" reste à ~5 secondes
(la connexion tombe réellement toutes les ~5s, ce n'est pas un artefact du
throttle — ce point était resté ambigu plus tôt dans la session). **Mais**
la reconnexion après chaque coupure est maintenant quasi-instantanée (300ms
au lieu de 5000ms) — le ratio temps-connecté/temps-total passe d'un
équilibre initialement défavorable à environ **4,7s connecté sur 5s**, soit
un gain net. Une publication réussie a été observée pendant une fenêtre
active (`publish tx ... ok=1`), confirmant que les données passent bien
durant les périodes de connexion.

**Ce correctif ne résout pas la cause** (toujours inconnue), mais réduit
fortement la fenêtre de perte de données à chaque cycle — objectif
pragmatique atteint en attendant une investigation plus poussée.

## Vérifications structurelles (taille et ordre d'initialisation)
Deux vérifications concrètes, à très faible risque, ont été faites sur le
firmware réel (déjà fonctionnel) :

**Taille de l'objet** — `sizeof(MyMesh) = 14576 octets`,
`sizeof(MqttBridge) = 1484 octets`. Pour un ESP32-S3 disposant de 512 Ko de
SRAM interne + 2 Mo de PSRAM, 14,5 Ko est **modeste**, rien d'alarmant en
soi. **Cette donnée affaiblit l'hypothèse "objet trop gros" comme cause
directe.**

**Ordre d'initialisation des membres** — inspection de `MyMesh.h` : `bridge`
(`MqttBridge`) est déclaré **tout à la fin** de la classe, bien après
`_prefs`. En C++, les membres sont construits dans l'ordre de déclaration,
donc `_prefs` est déjà pleinement construit avant `bridge`. Par ailleurs,
`_mgr` et `rtc` (les deux autres arguments passés au constructeur de
`MqttBridge`) proviennent de la classe de base `mesh::Mesh`, elle-même
toujours construite intégralement avant n'importe quel membre de la classe
dérivée `MyMesh`, conformément aux règles standards du C++. **L'hypothèse
d'un bug d'ordre d'initialisation est donc écartée** — le code est correct
sur ce point.

**Conclusion** : la piste structurelle "membre de `MyMesh`" reste la seule
différence connue entre sketch isolé (stable) et firmware complet (buggy),
mais ni la taille de l'objet ni l'ordre d'initialisation n'expliquent
pourquoi. Le mécanisme exact reste à ce stade **non identifié** — il
faudrait des outils d'investigation plus poussés que de simples
`Serial.printf()` pour aller plus loin (par exemple : débogueur JTAG,
activation des logs bas niveau de la pile réseau lwIP/ESP-IDF, ou
comparaison de la table des symboles/mémoire entre les deux builds).
